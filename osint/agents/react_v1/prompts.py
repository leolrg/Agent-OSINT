...
_ROUTING_RULES = {
    ...
    "web_extract": "web_extract — fetch full Markdown content of one or more URLs (apify/website-content-crawler, HTTP-only cheerio mode). Use after web_search on URLs that look promising. CANNOT extract scraper-blocked / login-walled origins (returns 403 or empty): linkedin.com, instagram.com, facebook.com, tiktok.com, x.com, twitter.com, threads.net, zhihu.com, weibo.com, xiaohongshu.com. For LinkedIn URLs route to apify_linkedin; Instagram → apify_instagram; X/Twitter → apify_twitter; Xiaohongshu (xiaohongshu.com) → apify_xiaohongshu (note: it's a keyword-search tool, not a URL-fetch — search for the relevant author/keyword instead). For Zhihu/Weibo, the search snippet is your best evidence — it often inlines the data anyway.",
    ...
    "apify_xiaohongshu": "apify_xiaohongshu — search Xiaohongshu (小红书 / RedNote) by keyword. Use for any subject with plausible Chinese-platform presence (CN-name students, lifestyle/fashion/wellness/travel/food interests, or Web3 users who cross-post). web_search does NOT index Xiaohongshu reliably. Pass `query` (Chinese characters or pinyin work best — Latin spellings of CN names usually miss). Returns post URL + author nickname + author user_id (durable handle — load profile via xiaohongshu.com/user/profile/<user_id>) + like count. Each call returns 100 results minimum at ~$0.50 — use sparingly and only when CN-platform presence is plausible.",
}
