...
async def test_cli_enable_is_additive_to_defaults(tmp_path: Path):
    """--enable adds tools to the default free set; doesn't replace it."""
    fake = type("R", (), {})()
    fake.scan_id = "sid"
    fake.path = tmp_path / "sid.json"
    (tmp_path / "sid.json").write_text("{}")
    with patch("osint.cli.scan", new=AsyncMock(return_value=fake)) as m:
        await main([
            "scan", "Jane", "--scans-dir", str(tmp_path),
            "--enable", "apify_instagram",
            "--enable", "apify_linkedin",
        ])
    cfg = m.call_args.kwargs["config"]
    # Defaults stayed on, plus the two opt-ins.
    assert cfg.enabled_tools == {
        "tavily_search", "tavily_extract", "maigret",
        "apify_instagram", "apify_linkedin",
    }


async def test_cli_only_replaces_default_set(tmp_path: Path):
    """--only sets the exact tool set, dropping defaults."""
    fake = type("R", (), {})()
    fake.scan_id = "sid"
    fake.path = tmp_path / "sid.json"
    (tmp_path / "sid.json").write_text("{}")
    ...
    cfg = m.call_args.kwargs["config"]
    assert cfg.enabled_tools == {"apify_twitter"}


async def test_cli_enable_and_only_mutually_exclusive(tmp_path: Path):
    """--enable and --only together should fail fast."""
    with pytest.raises(SystemExit) as exc:
        await main([
            "scan", "Jane", "--scans-dir", str(tmp_path),
            "--enable", "apify_instagram",
